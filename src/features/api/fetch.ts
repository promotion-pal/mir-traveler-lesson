...
import { getTokens } from "../data/token";
...
export class FetchApi {
  ...
  public async uploadFiles<T>(url: string, formData: FormData): Promise<T> {
    const { accessToken } = await getTokens();
  ...
  public async updateFiles<T>(url: string, formData: FormData): Promise<T> {
    const { accessToken } = await getTokens();
